...
{
    [SerializeField]
    private List<GameObject> placedGameObject = new List<GameObject>(); // 배치된 게임 오브젝트들을 저장하는 리스트

    // 오브젝트를 배치하고 해당 오브젝트의 인덱스를 반환하는 메서드
    public int Placeobject(GameObject prefab, Vector3 position)
    {
        GameObject newObject = Instantiate(prefab,position, Quaternion.identity); // 프리팹으로부터 새 오브젝트 생성
