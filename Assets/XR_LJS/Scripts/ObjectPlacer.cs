...
{
    [SerializeField]
    private List<GameObject> placedGameObjects = new List<GameObject> (); // 배치된 게임 오브젝트들을 저장하는 리스트

    public int PlaceObject(GameObject prefab, Vector3 position, Quaternion rotation)
    {
        // 프리팹을 인스턴스화하여 지정된 위치에 오브젝트를 배치하는 메서드
        GameObject newObject = Instantiate(prefab, position, rotation); // 프리팹을 복제하여 새 오브젝트 생성
